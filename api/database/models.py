...
def init_db():
    ...
    try:
        if USE_POSTGRESQL:
            ...
            # collect_on_delivery_fee 컬럼 추가 (착불 택배비, 없는 경우에만)
            try:
                cursor.execute('ALTER TABLE settlements ADD COLUMN collect_on_delivery_fee INTEGER DEFAULT 0')
                print("[성공] settlements 테이블에 collect_on_delivery_fee 컬럼이 추가되었습니다.")
            except Exception as e:
                if 'duplicate column' not in str(e).lower() and 'already exists' not in str(e).lower():
