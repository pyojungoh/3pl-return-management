...
def upload_settlement_file():
    ...
    try:
        ...
        # 파일 업로드: Google Drive 시도 → 실패 시 Cloudinary (작업비/입출고비/택배비 동일 처리)
        file_url = None
        try:
            from api.uploads.oauth_drive import upload_settlement_excel_to_drive
            ...
            # Google Drive 실패 시 Cloudinary로 폴백 (OAuth 만료 등)
            print(f"[정보] Google Drive 업로드 실패, Cloudinary로 폴백: {gd_err}")
            try:
                ...
                return jsonify({
                    'success': False,
                    'message': f'파일 업로드 실패 (Google Drive: {gd_err}, Cloudinary: {cl_err})'
                }), 500
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return jsonify({
                    'success': False,
                    'message': f'세금계산서 업로드 실패 (Google Drive: {gd_err}, Cloudinary: {cl_err})'
                }), 500
