# -*- coding: utf-8 -*-
"""One-off patch for pallets.html QR batch print + reliable print timing."""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parent
path = ROOT / "pallets.html"
text = path.read_text(encoding="utf-8")

# 1) Insert batch QR button after delete-pallets filter group
pat_btn = re.compile(
    r'(\n\s*<div class="pal-filter-group pal-admin-only">\s*\n\s*'
    r'<button class="pal-btn" onclick="palDeleteSelectedPallets\(\)"[^>]*>'
    r'[^<]*</button>\s*\n\s*</div>)'
    r'(\s*\n\s*</div>\s*\n\s*\n\s*<div class="pal-table-container">)',
    re.M,
)
m = pat_btn.search(text)
if not m:
    raise SystemExit("Could not find delete-pallets button block")
insert_btn = (
    m.group(1)
    + "\n          <div class=\"pal-filter-group pal-admin-only\">\n"
    + "            <button type=\"button\" class=\"pal-btn\" onclick=\"palPrintSelectedQrLabels()\" "
    + "style=\"height: 40px; background: #17a2b8; color: white;\">"
    + "\u5339\ub2e8 QR \uc778\uc1c4</button>\n"
    + "          </div>"
    + m.group(2)
)
# Fix: I used wrong unicode for "선택" - should be \uc120\ud0dd
insert_btn = insert_btn.replace("\u5339\ub2e8", "\uc120\ud0dd")
text = pat_btn.sub(insert_btn, text, count=1)
...
HELPERS = r'''
    ...
    async function palPrintSelectedQrLabels() {
      ...
      if (ids.length > 80) {
        if (!confirm(ids.length + '\uac74 \uc785\ub2c8\ub2e4. \uacc4\uc18d\ud558\uc2dc\uaca0\uc2b5\ub2c8\uae4c?')) return;
      }
      var parts = [];
      try {
...
# 2) Insert helpers after palEscapeHtml
anchor = "      return div.innerHTML;\n    }\n\n    /**\n     * ��신일 계산"
if anchor not in text:
    raise SystemExit("palEscapeHtml anchor missing")
text = text.replace(
    anchor,
...
# 3) Globals after palCurrentQRInDate
...
if g_anchor not in text:
    raise SystemExit("QR globals anchor missing")
text = text.replace(
    g_anchor,
...
v_old = """    async function palViewQRCode(palletId, inDate) {
      try {
        ...
        if (modal && titleEl && inDateEl && qrContainer) {
          ...
          // Google Apps Script와 동일한 형식: 파레트ID, 작업유형(보관종료), ��주사, ���목명 포함
...
if c_old not in text:
    raise SystemExit("palClose anchor not found")
text = text.replace(
    c_old,
...
# 6) Replace palPrintQRCode ... until "// 모달 ���"
p_start = text.find("    //�� (63.5mm * 70mm 라��)\n    async function palPrintQRCode() {")
if p_start < 0:
    raise SystemExit("palPrintQRCode start not found")
p_end = text.find("\n    // 모달 ��부 ���� 시 ��기", p_start)
if p_end < 0:
...
p_new = """    //�� (63.5mm * 70mm 라��)
    async function palPrintQRCode() {
      ...
      try {
        ...
        console.error('QR� 오류:', error);
        alert('QR 코드 인��� 중 오류가 발생했습니다.');
      }
    }"""

text = text[:p_start] + p_new + text[p_end:]
