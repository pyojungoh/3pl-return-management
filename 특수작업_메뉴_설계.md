# 특수작업 메뉴 설계 문서

## 📋 목차
1. [핵심 원칙](#핵심-원칙)
2. [파일 구조](#파일-구조)
3. [모드별 함수 분리 구조](#모드별-함수-분리-구조)
4. [HTML 구조](#html-구조)
5. [초기화 흐름](#초기화-흐름)
6. [가시성 제어](#가시성-제어)
7. [API 설계](#api-설계)
8. [데이터베이스 설계](#데이터베이스-설계)
9. [작업 순서](#작업-순서)

---

## 핵심 원칙

### ⚠️ 모드 완전 분리 원칙
- **각 모드는 독립적인 함수 그룹으로 완전 분리**
- **한 모드 수정이 다른 모드에 영향을 주지 않도록 설계**
- **공통 기능은 별도 모듈로 분리하여 재사용**

### 모드 구분
1. **관리자 모드**: 작업 종류 관리, 작업 등록, 모든 화주사 데이터 조회
2. **화주사 모드**: 본인 화주사 데이터만 조회
3. **공통 모드**: 작업 목록 테이블, 기본 스타일, 공통 유틸리티

---

## 파일 구조

```text
프로젝트 루트/
├── special_works.html (새 파일)
│   ├── HTML 구조
│   │   ├── 관리자 전용 섹션 (swAdminSection)
│   │   ├── 화주사 전용 섹션 (swConsignorSection)
│   │   └── 공통 섹션 (swCommonSection)
│   ├── CSS (모드별 스타일 분리)
│   └── JavaScript (모드별 함수 분리)
│       ├── 공통 함수 (swCommon*)
│       ├── 관리자 함수 (swAdmin*)
│       └── 화주사 함수 (swConsignor*)
│
├── api/
│   └── special_works/ (새 디렉토리)
│       ├── __init__.py
│       └── routes_db.py (새 파일)
│           └── API 엔드포인트 (모드 구분 없이 데이터만 제공)
│
├── app.py (기존 파일, blueprint 등록만 추가)
└── dashboard_server.html (기존 파일, 메뉴 버튼만 추가)
```

---

## 모드별 함수 분리 구조

### 1. 초기화 함수 (완전 분리)

#### 공통 초기화
```javascript
/**
 * 공통 초기화 함수
 * 역할 확인 후 모드별 초기화 함수 호출
 */
function swCommonInit() {
  // 1. 컨테이너 확인
  const container = document.getElementById('swContainer');
  if (!container) {
    console.error('[SW] swContainer를 찾을 수 없습니다');
    return;
  }
  
  // 2. 역할 확인 (전역 변수 우선 사용)
  const role = window.currentRole || '';
  const company = window.currentCompany || '';
  const username = window.currentUsername || '';
  
  // 3. 전역 변수 설정
  swCurrentRole = role;
  swCurrentCompany = company;
  swCurrentUsername = username;
  
  // 4. 모드별 초기화 함수 호출
  if (swCurrentRole === '관리자') {
    swAdminInit();
  } else {
    swConsignorInit();
  }
  
  // 5. 가시성 제어
  swApplyVisibility();
}
```

#### 관리자 전용 초기화
```javascript
/**
 * 관리자 모드 초기화 함수
 * 관리자 전용 기능만 초기화
 */
function swAdminInit() {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  // 1. 날짜 필터 초기화 (오늘 날짜)
  const today = new Date().toISOString().split('T')[0];
  const startDate = container.querySelector('#swAdminFilterStartDate');
  const endDate = container.querySelector('#swAdminFilterEndDate');
  if (startDate) startDate.value = today;
  if (endDate) endDate.value = today;
  
  // 2. 작업 등록 폼 날짜 초기화
  const workDate = container.querySelector('#swAdminWorkDate');
  if (workDate) workDate.value = today;
  
  // 3. 화주사 목록 로드
  swAdminLoadCompanies();
  
  // 4. 작업 종류 목록 로드
  swCommonLoadWorkTypes();
  
  // 5. 작업 목록 로드
  swAdminLoadWorks();
}
```

#### 화주사 전용 초기화
```javascript
/**
 * 화주사 모드 초기화 함수
 * 화주사 전용 기능만 초기화
 */
function swConsignorInit() {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  // 1. 월 필터 초기화 (오늘 월)
  const today = new Date().toISOString().split('T')[0];
  const monthInput = container.querySelector('#swConsignorFilterMonth');
  if (monthInput) monthInput.value = today.slice(0, 7); // YYYY-MM
  
  // 2. 작업 종류 목록 로드
  swCommonLoadWorkTypes();
  
  // 3. 작업 목록 로드
  swConsignorLoadWorks();
}
```

### 2. 데이터 로드 함수 (완전 분리)

#### 관리자 전용 데이터 로드
```javascript
/**
 * 관리자 모드 작업 목록 로드
 * 관리자 필터 기준으로 데이터 조회
 */
function swAdminLoadWorks() {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  // 1. 필터 값 수집
  const startDate = container.querySelector('#swAdminFilterStartDate')?.value || '';
  const endDate = container.querySelector('#swAdminFilterEndDate')?.value || '';
  const workTypeId = container.querySelector('#swAdminFilterWorkType')?.value || '';
  const companyName = container.querySelector('#swAdminFilterCompany')?.value || '';
  
  // 2. 날짜가 비어있으면 오늘 날짜로 설정
  const today = new Date().toISOString().split('T')[0];
  const finalStartDate = startDate || today;
  const finalEndDate = endDate || today;
  
  // 3. API URL 구성
  let url = '/api/special-works/works?';
  if (finalStartDate) url += `start_date=${finalStartDate}&`;
  if (finalEndDate) url += `end_date=${finalEndDate}&`;
  if (workTypeId) url += `work_type_id=${workTypeId}&`;
  if (companyName) url += `company_name=${encodeURIComponent(companyName)}&`;
  
  // 4. 로딩 표시
  const tbody = container.querySelector('#swTableBody');
  if (tbody) tbody.innerHTML = '<tr><td colspan="10">로딩 중...</td></tr>';
  
  // 5. API 호출
  fetch(url, {
    credentials: 'include',
    headers: swGetUserHeaders()
  })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        let works = data.data || [];
        // 날짜 내림차순 정렬
        works.sort((a, b) => new Date(b.work_date) - new Date(a.work_date));
        // 관리자 전용 렌더링
        swAdminRenderWorks(works);
        // 관리자 전용 통계 업데이트
        swAdminUpdateStats(works);
      }
    })
    .catch(error => {
      console.error('[SW] 관리자 작업 목록 로드 오류:', error);
    });
}
```

#### 화주사 전용 데이터 로드
```javascript
/**
 * 화주사 모드 작업 목록 로드
 * 화주사 필터 기준으로 데이터 조회
 */
function swConsignorLoadWorks() {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  // 1. 월 필터 값 수집
  const monthValue = container.querySelector('#swConsignorFilterMonth')?.value || '';
  
  // 2. 월을 날짜 범위로 변환
  let startDate = '';
  let endDate = '';
  if (monthValue) {
    const [yearStr, monthStr] = monthValue.split('-');
    const year = parseInt(yearStr, 10);
    const month = parseInt(monthStr, 10);
    const lastDay = new Date(year, month, 0).getDate();
    const paddedMonth = monthStr.padStart(2, '0');
    startDate = `${yearStr}-${paddedMonth}-01`;
    endDate = `${yearStr}-${paddedMonth}-${String(lastDay).padStart(2, '0')}`;
  }
  
  // 3. 화주사명 확인
  const companyName = swCurrentCompany;
  if (!companyName) {
    console.error('[SW] 화주사 정보를 확인할 수 없습니다');
    return;
  }
  
  // 4. API URL 구성
  let url = '/api/special-works/works?';
  if (startDate) url += `start_date=${startDate}&`;
  if (endDate) url += `end_date=${endDate}&`;
  url += `company_name=${encodeURIComponent(companyName)}&`;
  
  // 5. 로딩 표시
  const tbody = container.querySelector('#swTableBody');
  if (tbody) tbody.innerHTML = '<tr><td colspan="9">로딩 중...</td></tr>';
  
  // 6. API 호출
  fetch(url, {
    credentials: 'include',
    headers: swGetUserHeaders()
  })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        let works = data.data || [];
        // 날짜 내림차순 정렬
        works.sort((a, b) => new Date(b.work_date) - new Date(a.work_date));
        // 화주사 전용 렌더링
        swConsignorRenderWorks(works);
        // 화주사 전용 통계 업데이트
        swConsignorUpdateStats(works);
      }
    })
    .catch(error => {
      console.error('[SW] 화주사 작업 목록 로드 오류:', error);
    });
}
```

### 3. 렌더링 함수 (완전 분리)

#### 관리자 전용 렌더링
```javascript
/**
 * 관리자 모드 작업 목록 렌더링
 * 관리 컬럼 포함
 */
function swAdminRenderWorks(works) {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  const tbody = container.querySelector('#swTableBody');
  if (!tbody) return;
  
  // 작업 종류 필터 적용
  let filteredWorks = works;
  if (swSelectedWorkType) {
    filteredWorks = works.filter(work => work.work_type_name === swSelectedWorkType);
  }
  
  if (filteredWorks.length === 0) {
    tbody.innerHTML = '<tr><td colspan="10">등록된 작업이 없습니다.</td></tr>';
    return;
  }
  
  // HTML 생성 (관리 컬럼 포함)
  const html = filteredWorks.map((work, index) => {
    // ... HTML 생성 로직
    return `
      <tr>
        <td>${index + 1}</td>
        <td>${work.work_date}</td>
        <td>${work.company_name}</td>
        <td>${work.work_type_name}</td>
        <td>${work.quantity}</td>
        <td>${work.unit_price}원</td>
        <td>${work.total_price}원</td>
        <td>${photoThumbnails}</td>
        <td>${work.memo}</td>
        <td class="sw-admin-only">
          <button onclick="swAdminDeleteWork(${work.id})">삭제</button>
        </td>
      </tr>
    `;
  }).join('');
  
  tbody.innerHTML = html;
  
  // 가시성 제어 (관리 컬럼 표시)
  swApplyVisibility();
}
```

#### 화주사 전용 렌더링
```javascript
/**
 * 화주사 모드 작업 목록 렌더링
 * 관리 컬럼 제외
 */
function swConsignorRenderWorks(works) {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  const tbody = container.querySelector('#swTableBody');
  if (!tbody) return;
  
  // 작업 종류 필터 적용
  let filteredWorks = works;
  if (swSelectedWorkType) {
    filteredWorks = works.filter(work => work.work_type_name === swSelectedWorkType);
  }
  
  if (filteredWorks.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9">등록된 작업이 없습니다.</td></tr>';
    return;
  }
  
  // HTML 생성 (관리 컬럼 제외)
  const html = filteredWorks.map((work, index) => {
    // ... HTML 생성 로직
    return `
      <tr>
        <td>${index + 1}</td>
        <td>${work.work_date}</td>
        <td>${work.work_type_name}</td>
        <td>${work.quantity}</td>
        <td>${work.unit_price}원</td>
        <td>${work.total_price}원</td>
        <td>${photoThumbnails}</td>
        <td>${work.memo}</td>
      </tr>
    `;
  }).join('');
  
  tbody.innerHTML = html;
  
  // 가시성 제어 (관리 컬럼 숨김)
  swApplyVisibility();
}
```

### 4. 통계 업데이트 함수 (완전 분리)

#### 관리자 전용 통계
```javascript
/**
 * 관리자 모드 작업 종류별 통계 업데이트
 */
function swAdminUpdateStats(works) {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  const statsContainer = container.querySelector('#swAdminStatsContainer');
  if (!statsContainer) return;
  
  // 작업 종류별 그룹화
  const stats = {};
  works.forEach(work => {
    const typeName = work.work_type_name || '기타';
    if (!stats[typeName]) {
      stats[typeName] = { count: 0, total: 0 };
    }
    stats[typeName].count++;
    stats[typeName].total += work.total_price || 0;
  });
  
  // HTML 생성
  const html = Object.entries(stats).map(([typeName, data]) => {
    return `
      <div class="sw-stat-box" onclick="swFilterByWorkType('${typeName}')">
        ${typeName} / ${data.count}건 / 합계: ${data.total.toLocaleString()}원
      </div>
    `;
  }).join('');
  
  statsContainer.innerHTML = html;
}
```

#### 화주사 전용 통계
```javascript
/**
 * 화주사 모드 작업 종류별 통계 업데이트
 */
function swConsignorUpdateStats(works) {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  const statsContainer = container.querySelector('#swConsignorStatsContainer');
  if (!statsContainer) return;
  
  // 작업 종류별 그룹화
  const stats = {};
  works.forEach(work => {
    const typeName = work.work_type_name || '기타';
    if (!stats[typeName]) {
      stats[typeName] = { count: 0, total: 0 };
    }
    stats[typeName].count++;
    stats[typeName].total += work.total_price || 0;
  });
  
  // HTML 생성
  const html = Object.entries(stats).map(([typeName, data]) => {
    return `
      <div class="sw-stat-box" onclick="swFilterByWorkType('${typeName}')">
        ${typeName} / ${data.count}건 / 합계: ${data.total.toLocaleString()}원
      </div>
    `;
  }).join('');
  
  statsContainer.innerHTML = html;
}
```

### 5. 공통 함수

#### 작업 종류 목록 로드 (공통)
```javascript
/**
 * 작업 종류 목록 로드 (공통)
 * 관리자와 화주사 모두 사용
 */
function swCommonLoadWorkTypes() {
  fetch('/api/special-works/types', {
    credentials: 'include',
    headers: swGetUserHeaders()
  })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        swWorkTypes = data.data || [];
        // 관리자 모드면 작업 종류 관리 목록 업데이트
        if (swCurrentRole === '관리자') {
          swAdminUpdateWorkTypeList(swWorkTypes);
        }
        // 작업 종류 선택 드롭다운 업데이트 (관리자 작업 등록 폼)
        if (swCurrentRole === '관리자') {
          swAdminUpdateWorkTypeSelects(swWorkTypes);
        }
      }
    })
    .catch(error => {
      console.error('[SW] 작업 종류 로드 오류:', error);
    });
}
```

#### 가시성 제어 (공통)
```javascript
/**
 * 모드별 가시성 제어
 * 관리자/화주사 섹션 표시/숨김
 */
function swApplyVisibility() {
  const container = document.getElementById('swContainer');
  if (!container) return;
  
  const isAdmin = swCurrentRole === '관리자';
  
  // 관리자 섹션
  container.querySelectorAll('.sw-admin-only').forEach(el => {
    el.style.display = isAdmin ? '' : 'none';
  });
  
  // 화주사 섹션
  container.querySelectorAll('.sw-consignor-only').forEach(el => {
    el.style.display = isAdmin ? 'none' : '';
  });
}
```

---

## HTML 구조

### 전체 컨테이너
```html
<div id="swContainer">
  <!-- 공통 섹션: 작업 종류 관리, 작업 등록 (관리자만 표시) -->
  <div id="swCommonSection">
    <!-- 작업 종류 관리 (관리자만) -->
    <div id="swAdminWorkTypeSection" class="sw-admin-only">
      <button onclick="swAdminToggleWorkTypeManagement()">⚙️ 작업 종류 관리</button>
      <div id="swAdminWorkTypeManagement" style="display: none;">
        <!-- 작업 종류 목록 -->
      </div>
    </div>
    
    <!-- 작업 등록 (관리자만) -->
    <div id="swAdminWorkRegistrationSection" class="sw-admin-only">
      <button onclick="swAdminToggleWorkRegistration()">➕ 작업 등록</button>
      <div id="swAdminWorkRegistration" style="display: none;">
        <!-- 작업 등록 폼 -->
      </div>
    </div>
  </div>
  
  <!-- 관리자 전용 필터 섹션 -->
  <div id="swAdminFilterSection" class="sw-admin-only">
    <div>
      <label>시작일</label>
      <input type="date" id="swAdminFilterStartDate">
    </div>
    <div>
      <label>종료일</label>
      <input type="date" id="swAdminFilterEndDate">
    </div>
    <div>
      <label>작업 종류</label>
      <select id="swAdminFilterWorkType">
        <option value="">전체</option>
      </select>
    </div>
    <div>
      <label>화주사</label>
      <select id="swAdminFilterCompany">
        <option value="">전체 화주사</option>
      </select>
    </div>
    <button onclick="swAdminLoadWorks()">조회</button>
    <div id="swAdminStatsContainer"></div>
  </div>
  
  <!-- 화주사 전용 필터 섹션 -->
  <div id="swConsignorFilterSection" class="sw-consignor-only">
    <div>
      <label>조회 월</label>
      <input type="month" id="swConsignorFilterMonth">
    </div>
    <button onclick="swConsignorLoadWorks()">조회</button>
    <div id="swConsignorStatsContainer"></div>
  </div>
  
  <!-- 공통 작업 목록 테이블 -->
  <div id="swCommonTableSection">
    <div class="table-container">
      <table>
        <thead>
          <tr>
            <th>순번</th>
            <th>작업 일자</th>
            <th class="sw-admin-only">화주사</th>
            <th>작업 종류</th>
            <th>수량</th>
            <th>단가</th>
            <th>총액</th>
            <th>사진</th>
            <th>메모</th>
            <th class="sw-admin-only">관리</th>
          </tr>
        </thead>
        <tbody id="swTableBody">
          <tr>
            <td colspan="10">로딩 중...</td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
</div>
```

---

## 초기화 흐름

```text
1. 페이지 로드 또는 메뉴 클릭
   ↓
2. swCommonInit() 호출
   ├─ 컨테이너 확인
   ├─ 역할 확인 (window.currentRole)
   ├─ 전역 변수 설정
   └─ 모드별 초기화 함수 호출
      ├─ 관리자 → swAdminInit()
      │   ├─ 날짜 필터 초기화
      │   ├─ swAdminLoadCompanies()
      │   ├─ swCommonLoadWorkTypes()
      │   └─ swAdminLoadWorks()
      │
      └─ 화주사 → swConsignorInit()
          ├─ 월 필터 초기화
          ├─ swCommonLoadWorkTypes()
          └─ swConsignorLoadWorks()
   ↓
3. swApplyVisibility() 호출
   ├─ 관리자 섹션 표시/숨김
   └─ 화주사 섹션 표시/숨김
   ↓
4. 모드별 데이터 로드
   ├─ 관리자 → swAdminLoadWorks()
   │   └─ swAdminRenderWorks()
   │   └─ swAdminUpdateStats()
   │
   └─ 화주사 → swConsignorLoadWorks()
       └─ swConsignorRenderWorks()
       └─ swConsignorUpdateStats()
```

---

## 가시성 제어

### CSS 클래스
```css
/* 관리자 전용 */
.sw-admin-only {
  display: none; /* 기본 숨김 */
}

/* 화주사 전용 */
.sw-consignor-only {
  display: none; /* 기본 숨김 */
}
```

### JavaScript 가시성 제어
```javascript
function swApplyVisibility() {
  const isAdmin = swCurrentRole === '관리자';
  
  // 관리자 섹션
  document.querySelectorAll('.sw-admin-only').forEach(el => {
    el.style.display = isAdmin ? '' : 'none';
  });
  
  // 화주사 섹션
  document.querySelectorAll('.sw-consignor-only').forEach(el => {
    el.style.display = isAdmin ? 'none' : '';
  });
}
```

---

## API 설계

### 엔드포인트 목록

#### 작업 종류 API
- `GET /api/special-works/types` - 작업 종류 목록 조회
- `POST /api/special-works/types` - 작업 종류 생성
- `PUT /api/special-works/types/<id>` - 작업 종류 수정
- `DELETE /api/special-works/types/<id>` - 작업 종류 삭제

#### 작업 API
- `GET /api/special-works/works` - 작업 목록 조회
  - Query Parameters:
    - `start_date` (선택): 시작일 (YYYY-MM-DD)
    - `end_date` (선택): 종료일 (YYYY-MM-DD)
    - `work_type_id` (선택): 작업 종류 ID
    - `company_name` (선택): 화주사명 (관리자만 사용, 없으면 모든 화주사)
- `POST /api/special-works/works` - 작업 생성
- `GET /api/special-works/works/<id>` - 작업 상세 조회
- `PUT /api/special-works/works/<id>` - 작업 수정
- `DELETE /api/special-works/works/<id>` - 작업 삭제

### API 응답 형식
```json
{
  "success": true,
  "data": [...],
  "message": "성공 메시지"
}
```

---

## 데이터베이스 설계

### 테이블 구조

#### special_work_types (작업 종류)
- `id`: INTEGER PRIMARY KEY
- `name`: TEXT NOT NULL (작업 종류명)
- `default_unit_price`: INTEGER DEFAULT 0 (기본 단가)
- `display_order`: INTEGER DEFAULT 0 (표시 순서)
- `created_at`: TIMESTAMP
- `updated_at`: TIMESTAMP

#### special_works (작업)
- `id`: INTEGER PRIMARY KEY
- `company_name`: TEXT NOT NULL (화주사명)
- `work_type_id`: INTEGER NOT NULL (작업 종류 ID)
- `work_date`: DATE NOT NULL (작업 일자)
- `quantity`: REAL NOT NULL (수량)
- `unit_price`: INTEGER NOT NULL (단가)
- `total_price`: INTEGER NOT NULL (총액)
- `photo_links`: TEXT (사진 링크, 쉼표로 구분)
- `memo`: TEXT (메모)
- `created_at`: TIMESTAMP
- `updated_at`: TIMESTAMP

---

## 작업 순서

### 1단계: 파일 생성
- [ ] `special_works.html` 파일 생성
- [ ] `api/special_works/__init__.py` 파일 생성
- [ ] `api/special_works/routes_db.py` 파일 생성

### 2단계: HTML 구조 작성
- [ ] 컨테이너 구조 작성
- [ ] 관리자 섹션 작성
- [ ] 화주사 섹션 작성
- [ ] 공통 섹션 작성

### 3단계: CSS 작성
- [ ] 기본 스타일 작성
- [ ] 모드별 스타일 작성
- [ ] 반응형 스타일 작성

### 4단계: JavaScript 함수 작성
- [ ] 공통 함수 작성 (swCommon*)
- [ ] 관리자 함수 작성 (swAdmin*)
- [ ] 화주사 함수 작성 (swConsignor*)

### 5단계: API 작성
- [ ] 작업 종류 API 작성
- [ ] 작업 API 작성
- [ ] 에러 처리 추가

### 6단계: 통합
- [ ] `app.py`에 blueprint 등록
- [ ] `dashboard_server.html`에 메뉴 버튼 추가
- [ ] AJAX 로드 함수 작성

### 7단계: 테스트
- [ ] 관리자 모드 테스트
- [ ] 화주사 모드 테스트
- [ ] 기존 기능 영향 확인

---

## 함수명 규칙

### 접두사 규칙
- **공통 함수**: `swCommon*` (예: `swCommonInit`, `swCommonLoadWorkTypes`)
- **관리자 함수**: `swAdmin*` (예: `swAdminInit`, `swAdminLoadWorks`, `swAdminRenderWorks`)
- **화주사 함수**: `swConsignor*` (예: `swConsignorInit`, `swConsignorLoadWorks`, `swConsignorRenderWorks`)

### ID 규칙
- **컨테이너**: `swContainer`
- **관리자 섹션**: `swAdmin*` (예: `swAdminFilterSection`, `swAdminWorkTypeSection`)
- **화주사 섹션**: `swConsignor*` (예: `swConsignorFilterSection`)
- **공통 섹션**: `swCommon*` (예: `swCommonTableSection`)

### CSS 클래스 규칙
- **관리자 전용**: `.sw-admin-only`
- **화주사 전용**: `.sw-consignor-only`
- **공통 스타일**: `.sw-*` (예: `.sw-container`, `.sw-table`)

---

## 주의사항

### ⚠️ 모드 분리 원칙
- **절대 금지**: 관리자 함수에서 화주사 함수 호출 금지
- **절대 금지**: 화주사 함수에서 관리자 함수 호출 금지
- **허용**: 공통 함수는 모든 모드에서 호출 가능

### ⚠️ 기존 코드 수정 금지
- `dashboard_server.html`의 기존 함수 수정 금지
- `app.py`의 기존 라우트 수정 금지
- 기존 파일은 추가만 가능 (수정 금지)

### ⚠️ 충돌 방지
- 모든 ID, 함수명, CSS 클래스에 `sw` 접두사 사용
- 기존 코드와 충돌하지 않는지 `grep`으로 확인

---

## 체크리스트

### 작업 전
- [ ] 기존 파일을 수정하지 않고도 구현 가능한지 확인
- [ ] 모든 ID/함수명이 기존 것과 충돌하지 않는지 확인
- [ ] 모드별 함수가 완전히 분리되었는지 확인

### 작업 후
- [ ] `git diff`로 기존 파일이 수정되지 않았는지 확인
- [ ] 관리자 모드가 정상 작동하는지 테스트
- [ ] 화주사 모드가 정상 작동하는지 테스트
- [ ] 기존 기능이 정상 작동하는지 테스트

