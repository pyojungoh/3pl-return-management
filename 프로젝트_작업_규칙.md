...
### 🔍 코딩 전 전체 점검 원칙 (절대 원칙) - 2025-01-20 추가
- **절대 필수**: 모든 코딩 작업(신규 기능 추가, 기존 코드 수정 모두) 전에 **반드시 해당 관련 코딩을 전체 점검**한 후에 수정해야 함
- **점검 항목**:
  - [ ] 수정하려는 파일의 전체 구조 파악
  - [ ] 관련 함수들의 호출 관계 확인
  - [ ] 데이터 흐름 및 로직 이해
  ...
  - [ ] 프론트엔드와 백엔드 연동 방식 확인
- **점검 방법**:
  - 관련 파일 전체 읽기 (`read_file` 도구 사용)
  - 관련 함수 검색 (`grep`, `codebase_search` 도구 사용)
  - 데이터 흐름 추적 (API 호출 → 백엔드 처리 → DB 조회 → 응답 → 프론트엔드 렌더링)
  - 실제 DB 상태 확인 (필요시 스크립트 작성하여 확인)
- **점검 후 작업**:
  - 전체 구조를 이해한 후에만 수정 시작
  - 부분적 이해로 인한 잘못된 수정 방지
  - 원인을 정확히 파악한 후 해결 방법 도출
- **⚠️ 중요**: 점검 없이 바로 수정하면 안 됨. 반드시 전체 점검 후 수정할 것.
...
- **배경**: Vercel Hobby 플랜은 Cron Jobs를 하루 1회만 허용하므로 `vercel.json`의 `crons` 설정을 제거함. (현재 `crons: []`)
- **대안**: 무료 서비스 cron-job.org를 사용해 `/api/cs/check-notifications` 엔드포인트를 **1분마다** 호출.
- **⚠️ 주기 필수**: **반드시 1분마다** 호출해야 함
  - **취소건 알림**: 1분마다 체크 → 1분마다 알림
  - **일반 미처리 C/S**: 5분마다 알림 (스케줄러 내부 로직이 last_notification_at으로 5분 간격 제어)
  - 5분마다 호출하면 취소건 알림이 1분마다 오지 않음
- **환경 변수 추가**:
  1. Vercel → `Settings → Environment Variables`
  2. `CRON_SECRET` 생성 (예: `k7P3x9QwA2rL8s1V5tZ0nY4bC6mE7u`)
  3. Environment는 Production(필요 시 Preview/Development) 선택 후 저장
- **cron-job.org 설정 절차**:
  1. `Create Cronjob` → URL에 `https://jjaysolution.com/api/cs/check-notifications?secret=<CRON_SECRET>`
     - (헤더 설정이 어려우면 `?secret=` 쿼리 파라미터 사용 가능)
     - 또는 URL만 `https://jjaysolution.com/api/cs/check-notifications` 로 하고 Headers에 인증 추가
  2. **주기: `Every 1 minute`** (1분마다 - 필수!)
  3. `Advanced` 탭
     - `Request method`: `POST` 또는 `GET`
     - Headers 사용 시: `Authorization : Bearer <CRON_SECRET>`
     - Time zone: `Asia/Seoul`
  4. 저장 후 History 페이지에서 1분마다 호출되는지 확인
- **동작 확인**:
  - Vercel 로그에서 `🔄 [Cron] C/S 알림 체크 시작 (Vercel Cron Jobs)` 메시지가 보이면 성공
  - 취소건: 1분마다 텔레그램 알림
  - 일반 미처리 C/S: 5분마다 텔레그램 알림
...
### 구글 드라이브 OAuth 설정 (정산 파일 업로드) — 2026-01-26 추가
- **용도**: 정산관리 메뉴에서 상세내역 엑셀 파일을 Google Drive(제이제이솔루션 > 정산파일)에 업로드
- **필수 환경 변수** (Vercel):
  - `GOOGLE_OAUTH_CREDENTIALS_JSON`: credentials.json 전체 내용
  - `GOOGLE_OAUTH_TOKEN_JSON`: 로컬에서 인증 후 추출한 토큰 JSON
- **취득 방법**:
  1. **GOOGLE_OAUTH_CREDENTIALS_JSON**
     - Google Cloud Console (https://console.cloud.google.com/) → 프로젝트 `composite-dream-477907-c5`
     - API 및 서비스 → 사용자 인증 정보 → + 사용자 인증 정보 만들기 → OAuth 클라이언트 ID
     - 애플리케이션 유형: **데스크톱 앱** → 만들기 → 다운로드 아이콘(↓)으로 JSON 다운로드
     - ⚠️ **클라이언트 보안 비밀번호는 생성 직후 한 번만 표시됨**. 잃어버리면 "보안 비밀번호 추가"로 새로 생성 후 다시 인증해야 함
  2. **GOOGLE_OAUTH_TOKEN_JSON**
     - 다운로드가 아님. 로컬에서 OAuth 인증 후 `extract_oauth_token.py`로 추출
     - 절차:
       1. `credentials.json`을 프로젝트 루트에 저장
       2. `python app.py` 실행
       3. 브라우저에서 `http://localhost:5000/api/uploads/test/oauth-auth` 접속 (또는 test-excel-upload.html에서 업로드 시도)
       4. Google 로그인 → `token.pickle` 생성
       5. `python extract_oauth_token.py` 실행 → 출력된 JSON 전체가 `GOOGLE_OAUTH_TOKEN_JSON` 값
- **주의사항**:
  - OAuth에 사용하는 Google 계정이 **제이제이솔루션** 폴더를 소유하거나 공유받아 있어야 함
  - credentials/token 변경 시 두 환경 변수 모두 갱신 후 Vercel 재배포 필요
  - `oauth_token_for_vercel.json`, `credentials.json`, `token.pickle`은 .gitignore에 포함되어 Git 커밋 금지
...
## 🚨 새로운 메뉴 추가 시 데이터 로딩 격리 규칙 (2025-01-20 추가)

### ⚠️ 핵심 원칙
**새로운 메뉴를 추가할 때는 반드시 해당 메뉴에서만 데이터가 로드되고 표시되도록 격리해야 합니다.**
**다른 메뉴로 이동하면 해당 메뉴의 데이터는 반드시 숨겨지고 초기화 플래그가 리셋되어야 합니다.**

### 📋 문제 상황
- 새로운 메뉴를 추가한 후 다른 메뉴로 이동해도 이전 메뉴의 데이터가 계속 표시됨
- 정산 메뉴를 한 번 방문한 후 다른 메뉴로 이동해도 정산 목록이 계속 표시됨
- 메뉴 전환 시 초기화 플래그가 리셋되지 않아 중복 로딩 방지 로직이 작동하지 않음

### ✅ 필수 구현 사항

#### 1. 메뉴 전환 함수에 숨김 처리 추가 (필수)
**위치**: `dashboard_server.html`의 `switchMenu()` 함수
...
function switchMenu(menuName) {
  ...
  // ⚠️ 필수: 다른 메뉴의 컨테이너 숨김 처리
  // {새메뉴명} 전용 컨테이너 숨김 ({새메뉴명} 메뉴가 아닐 때)
  const {새메뉴명}Container = document.getElementById('{새메뉴명}Tab');
  if ({새메뉴명}Container && menuName !== '{새메뉴명}') {
    {새메뉴명}Container.classList.add('hidden');
    {새메뉴명}Container.style.display = 'none';
    // ⚠️ 필수: 초기화 플래그 리셋
    if (typeof window.{접두사}Initialized !== 'undefined') {
      window.{접두사}Initialized = false;
    }
    // ⚠️ 필수: 컨테이너도 숨기기
    const {접두사}Container = document.getElementById('{접두사}Container');
    if ({접두사}Container) {
      {접두사}Container.style.display = 'none';
    }
  }
}
```

#### 2. 관리자 탭 전환 함수에 숨김 처리 추가 (필수)
**위치**: `dashboard_server.html`의 `switchAdminTab()` 함수
...
function switchAdminTab(tabName) {
  ...
  // ⚠️ 필수: {새메뉴명} 탭이 아닐 때 초기화 플래그 리셋
  if (tabName !== '{새메뉴명}') {
    const {새메뉴명}Tab = document.getElementById('{새메뉴명}Tab');
    if ({새메뉴명}Tab) {
      if (typeof window.{접두사}Initialized !== 'undefined') {
        window.{접두사}Initialized = false;
      }
  ...
  // ⚠️ 필수: {새메뉴명} 탭으로 전환 시 탭 표시 및 초기화 플래그 리셋
  } else if (tabName === '{새메뉴명}') {
    const {새메뉴명}Tab = document.getElementById('{새메뉴명}Tab');
    if ({새메뉴명}Tab) {
      {새메뉴명}Tab.classList.remove('hidden');
      {새메뉴명}Tab.style.display = 'block';
    }
    if (typeof window.{접두사}Initialized !== 'undefined') {
      window.{접두사}Initialized = false;
    }
...
#### 3. 초기화 함수에서 메뉴 활성화 여부 확인 (필수)
**위치**: `{새메뉴명}.html`의 `{접두사}CommonInit()` 함수

**구현 패턴**:
```javascript
function {접두사}CommonInit() {
  // ⚠️ 필수: 메뉴가 활성화되어 있는지 확인
  const {새메뉴명}Tab = document.getElementById('{새메뉴명}Tab');
  const {접두사}Container = document.getElementById('{접두사}Container');
  
  if (!{새메뉴명}Tab || !{접두사}Container) {
    console.log('[{접두사}] 메뉴가 활성화되지 않음, 초기화 건너뜀');
    return;
  }
  
  // ⚠️ 필수: 탭이 숨겨져 있으면 초기화하지 않음
  if ({새메뉴명}Tab.style.display === 'none' || {새메뉴명}Tab.classList.contains('hidden')) {
    console.log('[{접두사}] 탭이 숨겨져 있음, 초기화 건너뜀');
    return;
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', {접두사}CommonInit);
} else {
  {접두사}CommonInit();
}
```
...
**✅ 올바른 예시**:
```javascript
// ⚠️ 자동 초기화 제거 - dashboard_server.html의 load{새메뉴명}Content()에서만 호출
```

#### 5. load{새메뉴명}Content() 함수에서 조건부 초기화 (필수)
**위치**: `dashboard_server.html`의 `load{새메뉴명}Content()` 함수

**구현 패턴**:
```javascript
function load{새메뉴명}Content() {
  let container = document.getElementById('{새메뉴명}Tab');
  
  // ⚠️ 필수: 컨테이너가 존재하고, 현재 표시되고 있을 때만 로드
  if (container && container.style.display !== 'none' && !container.classList.contains('hidden')) {
    // ... HTML 로드 및 삽입 로직 ...
    
    setTimeout(() => {
      if (typeof window.{접두사}CommonInit === 'function') {
        // ⚠️ 필수: 다시 한 번 탭이 활성화되어 있는지 확인
        const {새메뉴명}Tab = document.getElementById('{새메뉴명}Tab');
        if ({새메뉴명}Tab && {새메뉴명}Tab.style.display !== 'none' && !{새메뉴명}Tab.classList.contains('hidden')) {
          window.{접두사}CommonInit();
        }
      }
    }, 100);
  } else {
    console.log('[{접두사}] {새메뉴명} 탭이 활성화되지 않아 로드하지 않습니다.');
  }
}
```

### ✅ 체크리스트 (새 메뉴 추가 시 반드시 확인)

#### 메뉴 전환 함수 체크리스트
- [ ] `switchMenu()` 함수에 새 메뉴 컨테이너 숨김 처리 추가
- [ ] 메뉴가 아닐 때 초기화 플래그 리셋 (`window.{접두사}Initialized = false`)
- [ ] 메뉴가 아닐 때 컨테이너 숨김 (`{접두사}Container.style.display = 'none'`)
- [ ] `switchAdminTab()` 함수에도 동일한 처리 추가 (관리자 탭인 경우)

#### 초기화 함수 체크리스트
- [ ] `{접두사}CommonInit()` 함수에서 메뉴 탭 존재 여부 확인
- [ ] `{접두사}CommonInit()` 함수에서 탭이 숨겨져 있으면 초기화하지 않음
- [ ] 중복 초기화 방지 플래그 (`{접두사}Initialized`) 사용

#### HTML 파일 체크리스트
- [ ] `{새메뉴명}.html`에서 `DOMContentLoaded` 자동 초기화 코드 제거
- [ ] 초기화 함수는 `dashboard_server.html`의 `load{새메뉴명}Content()`에서만 호출

#### 로드 함수 체크리스트
- [ ] `load{새메뉴명}Content()` 함수에서 탭 활성화 여부 확인
- [ ] 탭이 활성화된 상태에서만 HTML 로드 및 초기화 함수 호출
- [ ] 초기화 함수 호출 전에 다시 한 번 탭 활성화 여부 확인

### 🚨 자주 발생하는 실수

#### 실수 1: 메뉴 전환 시 숨김 처리 누락
**문제**: 다른 메뉴로 이동해도 이전 메뉴의 데이터가 계속 표시됨
**해결**: `switchMenu()` 또는 `switchAdminTab()` 함수에 반드시 숨김 처리 추가

#### 실수 2: 초기화 플래그 리셋 누락
**문제**: 초기화 플래그가 리셋되지 않아 중복 로딩 방지 로직이 작동하지 않음
**해결**: 메뉴 전환 시 `window.{접두사}Initialized = false` 추가

#### 실수 3: 자동 초기화 코드 미제거
**문제**: HTML 파일이 파싱될 때 자동으로 초기화되어 다른 메뉴에서도 데이터가 표시됨
**해결**: `{새메뉴명}.html`에서 `DOMContentLoaded` 이벤트 리스너 제거

#### 실수 4: 초기화 함수에서 탭 활성화 여부 미확인
**문제**: 탭이 숨겨진 상태에서도 초기화가 실행되어 데이터가 로드됨
**해결**: `{접두사}CommonInit()` 함수 시작 부분에 탭 활성화 여부 확인 추가

### 📚 참고 예시: 정산 메뉴 (settlements.html)
- **파일**: `settlements.html`, `dashboard_server.html`
- **접두사**: `st` (settlements)
- **구현 사항**:
  - `switchMenu()` 함수에 정산 탭 숨김 처리 추가
  - `switchAdminTab()` 함수에 정산 탭 숨김 처리 및 초기화 플래그 리셋 추가
  - `stCommonInit()` 함수에서 `settlementsTab` 활성화 여부 확인
  - `loadSettlementsContent()` 함수에서 탭 활성화 여부 확인 후 초기화
  - `settlements.html`에서 자동 초기화 코드 제거

### ⚠️ 중요 주의사항
1. **메뉴 전환 시 반드시 숨김 처리**: 다른 메뉴로 이동하면 이전 메뉴의 컨테이너를 반드시 숨겨야 함
2. **초기화 플래그 리셋 필수**: 메뉴 전환 시 초기화 플래그를 리셋하여 재초기화 가능하도록 해야 함
3. **자동 초기화 금지**: HTML 파일에서 자동 초기화 코드를 제거하고, `dashboard_server.html`에서만 호출
4. **활성화 여부 확인**: 초기화 함수와 로드 함수 모두에서 탭 활성화 여부를 확인해야 함
5. **테스트 필수**: 새 메뉴 추가 후 반드시 다른 메뉴로 이동해도 데이터가 표시되지 않는지 확인
