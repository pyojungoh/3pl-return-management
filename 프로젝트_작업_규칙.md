...
- **대안**: 무료 서비스 cron-job.org를 사용해 `/api/cs/check-notifications` 엔드포인트 호출.
- **권장: 1분용·5분용 두 개의 Cron Job 분리** (2026-03 추가)
  - 취소건과 일반건을 각각 별도 cron으로 호출하여 주기 분리
- **환경 변수 추가**:
  1. Vercel → `Settings → Environment Variables`
  2. `CRON_SECRET` 생성 (예: `k7P3x9QwA2rL8s1V5tZ0nY4bC6mE7u`)
  3. Environment는 Production(필요 시 Preview/Development) 선택 후 저장
- **cron-job.org 설정 절차** (두 개 생성, **URL 완전 분리**):
  - **Cron 1 (1분용 - 취소건)**:
    1. `Create Cronjob` → URL: `https://www.jjaysolution.com/api/cs/check-notifications-cancellation?secret=<CRON_SECRET>`
    2. **주기: `Every 1 minute`**
    3. Title: `C/S 취소건 알림 (1분)`
  - **Cron 2 (5분용 - 일반건)**:
    1. `Create Cronjob` → URL: `https://www.jjaysolution.com/api/cs/check-notifications-general?secret=<CRON_SECRET>`
    2. **주기: `Every 5 minutes`**
    3. Title: `C/S 일반건 알림 (5분)`
  - (기존) `/check-notifications?type=cancellation` 또는 `?type=general` 도 동작
- **동작 확인**:
  - History 페이지에서 각 cron이 주기대로 호출되는지 확인
  - 취소건: 1분마다 텔레그램 알림
